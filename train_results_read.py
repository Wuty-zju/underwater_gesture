...
def write_csv_and_print_results(base_directory, csv_path):
    ...
    for train_dir in tqdm(train_dirs, desc="Processing directories"):
        ...
        if os.path.isdir(full_train_dir):
            ...
            rows.append([train_dir.strip('train') if train_dir.strip('train') != '' else '1', model, batch, epoch, data_file_name, last_pt_path, best_pt_path, args_yaml_path, results_csv_path])
