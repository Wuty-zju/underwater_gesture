...
base_dir = os.path.normpath('datasets/CADDY_gestures')
input_csv = os.path.normpath('datasets/CADDY_gestures/CADDY_gestures_sample_dataset_v2.csv')
output_dir = os.path.normpath('datasets/CADDY_gestures_YOLO/datasets')
output_txt_dir = os.path.normpath('datasets/CADDY_gestures_YOLO')

# 定义训练集和验证集文件路径
train_file = os.path.normpath(os.path.join(output_txt_dir, 'train.txt'))
val_file = os.path.normpath(os.path.join(output_txt_dir, 'val.txt'))
...
# 读取数据集并排除包含“compressed”的条目
tp_df = pd.read_csv(input_csv)
tp_df = tp_df[~tp_df['stereo left'].str.contains('compressed') & ~tp_df['stereo right'].str.contains('compressed')]
...
def process_dataset(df, prefix, is_tp=True):
    ...
    for index, row in tqdm(df.iterrows(), total=len(df), desc=f"处理 {prefix.upper()} 数据集"):
        # 获取图像路径
...
def split_dataset():
    ...
    with open(train_file, 'w') as f:
        for left, right in tqdm(train_images, desc="保存训练集图像"):
            f.write(f'{left}\n')
            f.write(f'{right}\n')

    with open(val_file, 'w') as f:
        for left, right in tqdm(val_images, desc="保存验证集图像"):
            f.write(f'{left}\n')
            f.write(f'{right}\n')
...
yaml_file = os.path.normpath(os.path.join(output_txt_dir, 'CADDY_gestures.yaml').replace("\\", "/"))
